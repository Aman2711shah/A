...
class ReviewStepWidget extends StatelessWidget {
        ...
        Expanded(
          child: ListView(
            children: [
              ...
              _buildReviewSection(
                'Business Activity',
                _getActivityTitle(formData['selectedActivity']),
                Icons.business,
              ),
              ...
              _buildReviewSection(
                'Legal Structure',
                _getLegalStructureTitle(formData['legalStructure']),
              ...
              _buildReviewSection(
                'Shareholders',
                '${formData['numberOfShareholders'] ?? 0} shareholder${(formData['numberOfShareholders'] ?? 0) > 1 ? 's' : ''}',
                Icons.people,
              ),
              ...
              _buildReviewSection(
                'Visa Requirements',
                '${_getVisaTypeTitle(formData['visaType'])} (${formData['numberOfVisas'] ?? 0} visa${(formData['numberOfVisas'] ?? 0) != 1 ? 's' : ''})',
              ...
              _buildReviewSection(
                'Office Space',
                '${_getOfficeSpaceTitle(formData['officeType'])}${(formData['hasEjari'] ?? false) ? ' (with Ejari)' : ''}',
                Icons.location_city,
              ),
  ...
  Widget _buildTermsAndConditions() {
    final agreedToTerms = formData['agreedToTerms'] ?? false;
            ...
            children: [
              ...
              Expanded(
                child: GestureDetector(
                  onTap: () => onChanged('agreedToTerms', !agreedToTerms),
                  child: const Text(
  ...
  Map<String, double> _calculateCosts() {
    ...
    // Base license cost
    if (formData['legalStructure'] == 'llc') {
      costs['Business License'] = 15000.0;
    } else if (formData['legalStructure'] == 'sole_proprietorship') {
    ...
    // Visa costs
    if (formData['visaType'] != null && (formData['numberOfVisas'] ?? 0) > 0) {
      final visaCosts = {
      ...
      final costPerVisa = visaCosts[formData['visaType']] ?? 5000.0;
      costs['Visa Processing'] =
          costPerVisa * (formData['numberOfVisas'] ?? 0).toDouble();
    }
    ...
    if (formData['officeType'] != null) {
      ...
      costs['Office Space (Annual)'] =
          officeCosts[formData['officeType']] ?? 15000.0;
    }

    // Ejari costs
    if (!(formData['hasEjari'] ?? false) &&
